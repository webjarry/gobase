...
function wtJinDu($vo){
	...
	if($vo['status'] == 2){
		...
		$wtList = M('wtjoin')->where("tid=".$vo[id])->count();
		
		$choose = M('wtjoin')->where('tid='.$vo[id].' and choose=1')->find();
		...
		if(empty($choose)){
			...
			return wt_progress($vo['progress']);
...
//律师圈次数
function typeCount($id,$type){
	if($type == 1){
		$count = M('dongtai')->where("pid=".$id)->count();
		return empty($count)?0:$count;
	}elseif($type == 2){
		$count = M('pl')->where("tid=".$id." and type=2")->count();
		return empty($count)?0:$count;
	}else{
		$count = M('dz')->where("tid=".$id." and type=2")->count();
		return empty($count)?0:$count;
	}
}
