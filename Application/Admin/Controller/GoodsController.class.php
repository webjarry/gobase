...
class GoodsController extends AdminController {
	...
	public function _initialize(){
        ...
        $a=M('cate')->where($type)->where("pid=0")->order('sort asc')->select();
        foreach ($a as $k=>$v){
            $a[$k][son]=M('cate')->where($type)->where("pid=".$v[id])->order('sort asc')->select();
        }
		$this->assign("cate",$a);
		$this->assign("brand",M('brand')->where($type)->order('sort asc')->select());
		$this->assign("sery",M('sery')->where($type)->order('sort asc')->select());
        $this->assign("color",M('color')->where($type)->order('sort asc')->select());
        $this->assign("material",M('material')->where($type)->order('sort asc')->select());
		parent::_initialize();
	}
    ...
    public function index1(){
        ...
        //dump($map);
        $listsCount = M('goods')->alias('c')->join("bhy_price u on c.uid=u.id")->where($map)->count();
        $Page       = new \Think\Page($listsCount,20);
        $show       = $Page->show();

        $list = M('goods')->alias('c')->field("u.nickname,u.icon,c.*")
...
//echo M('fxcontent')->getLastSql();
    ...
    public function index(){
        ...
        //dump($map);
        $listsCount = M('goods')->where($map)->count();
...
//echo M('goods')->getLastSql();
    ...
    public function edit(){
        ...
        if(IS_POST){
            ...
            $result =M('goods')->where("id=".$id)->save($_POST);
            if($result){
            ...
            $map[id] = $id;
            //$map[pid] = array('eq',0);
            $editdata = D('goods')->where($map)->find();
    ...
    public function recommend(){
        $id=$_GET['id'];
        $recommend = M('goods')->where("id=".$id)->find()['recommend'];
        $tj=$recommend==1?0:1;
        $re=M('goods')->where("id=".$id)->save(array('recommend'=>$tj));
        if($re){

            $this->success('修改成功','/Admin/goods/index.html');
        }else{
            $this->error('修改失败');
        }
    }
    public function cheap(){
        $id=$_GET['id'];
        $recommend = M('goods')->where("id=".$id)->find()['cheap'];
        $tj=$recommend==1?0:1;
        $re=M('goods')->where("id=".$id)->save(array('cheap'=>$tj));
        if($re){
