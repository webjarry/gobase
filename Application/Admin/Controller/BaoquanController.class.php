 ...
 */
class BaoquanController extends AdminController {
    ...
    public function index1(){
        ...
        $prefix   = C('DB_PREFIX');
        $l_table  = $prefix.('baoquan');
        $r_table  = $prefix.('usermember');

        $model    = M()->table( $l_table.' o' )->join ( $r_table.' u ON u.id=uid' );
        //$_REQUEST = array();
        $list = $this->lists($model,$map,'id desc',null,'u.name,u.balance,*');
        int_to_string($list);
    ...
    public function index(){
        ...
        $d=M('baoquan')->where('status=1')->count();
        $this->assign('d',$d);
        $c=M('baoquan')->where('status=2')->count();
        $this->assign('c',$c);
        $s=M('baoquan')->where('status=3')->count();
        $this->assign('s',$s);
        $this->display();
    }
    //回复
    ...
    public function state(){
        ...
        $article_subgrade=M('baoquan')->where($id)->save($save);
        if($article_subgrade){
            $order=M('baoquan')->where("id=".$_GET['id'])->find();
            /*if($_GET['status'] ==2 ){
                if ($order[utype] == 1) {
                    $m=M('usermember');
