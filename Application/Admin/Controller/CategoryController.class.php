...
class CategoryController extends AdminController {
    ...
    public function index(){
       /* $result=M('order')->where('type=8')->select();
        foreach($result as $k=>$v){
             $wt = M('wt')->where("orderno=".$v[orderno])->find();
            if($wt){
                $c = M('wtjoin')->where("tid=".$wt[id]." and choose=1")->find();
                if($c){
                    $chajia = $c[money]-$wt[margin];
                    M('order')->where('id='.$v[id])->save(array('chajia'=>$chajia));
                }
        ...
        //echo time();
        $tree = D('Category')->getTree(0,'id,name,title,sort,pid,allow_publish,status,pagetype,web_url');
