 ...
 */
class WtController extends AdminController {
    ...
    public function index(){
        ...
        //dump($map);
        $listsCount = M('wt')->where($map)->count();
        $Page       = new \Think\Page($listsCount,20);
        $show       = $Page->show();

        $list = M('wt')->where($map)->order('create_time desc')->limit($Page->firstRow.','.$Page->listRows)->select();
//echo M('wt')->getLastSql();
        ...
        foreach($list as $k=>$v){
            ...
            $list[$k][ordertype]= M('ajcate')->find($v[ajid])['name'];

        }
        int_to_string($list);
        $this->assign('list', $list);

        $d=M('wt')->where('status=1')->count();
        $this->assign('d',$d);
        $c=M('wt')->where('status=2')->count();
        $this->assign('c',$c);
        $s=M('wt')->where('status=3')->count();
        $this->assign('s',$s);
		$wcount=M('wt')->where('status=4')->count();
    ...
    public function del(){
        ...
        $delResult = M('wt')->where($where)->delete();
    ...
    public function state(){
        $order=M('wt')->find($_GET['id']);

        $result=M('wt')->where("id=".$_GET['id'])->save(array('status'=>$_GET['status']));
        if($result){
            ...
            if($_GET['status'] ==2 ){
                ...
                if($order['type']==2){
                    ...
                    if(M('wt')->where("id=".$_GET['id'])->save(array('tic'=>$tic))){
