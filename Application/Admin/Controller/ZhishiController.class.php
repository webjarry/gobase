 ...
 */
class ZhishiController extends AdminController {
    ...
    public function index(){
        ...
        //dump($map);
        $listsCount = M('zhishi')->where($map)->count();
        $Page       = new \Think\Page($listsCount,20);
        $show       = $Page->show();

        $list = M('zhishi')->where($map)->order('create_time desc')->limit($Page->firstRow.','.$Page->listRows)->select();
//echo M('zhishi')->getLastSql();
        ...
        $d=M('zhishi')->where('status=1')->count();
        $this->assign('d',$d);
        $c=M('zhishi')->where('status=2')->count();
        $this->assign('c',$c);
        $s=M('zhishi')->where('status=3')->count();
        $this->assign('s',$s);
    ...
    public function edit(){
        if(IS_POST){
            ...
            $id = I('request.id');
            $map[id] = $id;
            $result = M('zhishi')->where($map)->find();
    ...
    public function del(){
        ...
        $delResult = M('zhishi')->where($where)->delete();
    ...
    public function state(){
        $order=M('zhishi')->find($_GET['id']);

        $result=M('zhishi')->where("id=".$_GET['id'])->save(array('status'=>$_GET['status']));
        if($result){
