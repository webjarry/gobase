 ...
 */
class TeamtypeController extends AdminController {
    ...
    public function index(){
        $tree = D('Producttype')->getTree(0,'id,title,sort,pid,status');
    ...
    public function edit($id = null, $pid = 0){
        ...
        if(IS_POST){ //提交表单
			...
			$bre['status'] = array('eq',1);
			$bre['pid'] = array('eq',5);
			
			$brand = M('category')->where($bre)->select();
    ...
    public function move(){
        $to = I('post.to');
        $from = I('post.from');
        $res = M('Producttype')->where(array('id'=>$from))->setField('pid', $to);
        if($res !== false){
