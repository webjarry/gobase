 ...
 */
class LegalpaperController extends AdminController {
    ...
    public function index(){
        ...
        //dump($map);
        $listsCount = M('legalpaper')->where($map)->count();
        $Page       = new \Think\Page($listsCount,20);
        $show       = $Page->show();

        $list = M('legalpaper')->where($map)->order('create_time desc')->limit($Page->firstRow.','.$Page->listRows)->select();
		
		
		foreach($list as $k => $v){
			
			$map['type'] = array('eq',16);
...
//echo M('legalpaper')->getLastSql();
        ...
        $d=M('legalpaper')->where('status=1')->count();
        $this->assign('d',$d);
        $c=M('legalpaper')->where('status=2')->count();
        $this->assign('c',$c);
        $s=M('legalpaper')->where('status=3')->count();
        $this->assign('s',$s);
    ...
    public function edit(){
        if(IS_POST){
            ...
            $id = I('request.id');
            $map[id] = $id;
            $result = M('legalpaper')->where($map)->find();
    ...
    public function del(){
        ...
        $delResult = M('legalpaper')->where($where)->delete();
    ...
    public function state(){
        $a=M('legalpaper')->find($_GET['id']);

        $result=M('legalpaper')->where("id=".$_GET['id'])->save(array('status'=>$_GET['status']));
        if($_GET['status']==3){
