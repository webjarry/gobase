 ...
 */
class CouponptController extends AdminController {
    ...
    public function index1(){
        ...
        //dump($map);
        $listsCount = M('couponpt')->alias('c')->join("bhy_price u on c.uid=u.id")->where($map)->count();
        $Page       = new \Think\Page($listsCount,20);
        $show       = $Page->show();

        $list = M('couponpt')->alias('c')->field("u.nickname,u.icon,c.*")
...
//echo M('fxcontent')->getLastSql();
    ...
    public function index(){
        ...
        //dump($map);
        $listsCount = M('couponpt')->where($map)->count();
        $Page       = new \Think\Page($listsCount,10);
        $show       = $Page->show();

        $list = M('couponpt')->where($map)->order('id desc')->limit($Page->firstRow.','.$Page->listRows)->select();
//echo M('couponpt')->getLastSql();
        foreach($list as $k=>$v){
            $type=5-$k;

            $list[$k][num]=M('couponjoin')->where("type=".$type)->count();
        }
        $this->assign('get',$_GET);
        $this->assign('_page',$show);
    ...
    public function add(){
        if(IS_POST){
            ...
            $map[id] = $id;
            //$map[pid] = array('eq',0);
            $editdata = M('discuz')->where($map)->find();
    ...
    public function edit(){
        ...
        if(IS_POST){
            ...
            $result =M('couponpt')->where("id=".$id)->save($_POST);
            if($result){
            ...
            $map[id] = $id;
            //$map[pid] = array('eq',0);
            $editdata = M('couponpt')->where($map)->find();
    ...
    public function recommend(){
        $id=$_GET['id'];
        $recommend = M('couponpt')->where("id=".$id)->find()['recommend'];
        $tj=$recommend==1?0:1;
        $re=M('couponpt')->where("id=".$id)->save(array('recommend'=>$tj));
        if($re){

            $this->success('修改成功','/Admin/couponpt/index.html');
        }else{
            $this->error('修改失败');
        }
    }
    public function cheap(){
        $id=$_GET['id'];
        $recommend = M('couponpt')->where("id=".$id)->find()['cheap'];
        $tj=$recommend==1?0:1;
        $re=M('couponpt')->where("id=".$id)->save(array('cheap'=>$tj));
        if($re){
