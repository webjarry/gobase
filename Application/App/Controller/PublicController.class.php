...
class PublicController extends HomeController {
	...
	public function uover(){
		...
		$map['orderno'] = array('eq','190528409221');
	...
	public function checkOpenid(){
		...
		if($result){
			...
			if($post['type'] == 1 && empty($result['unionid'])){
				
				M('usermember')->where('id='.$result['id'])->save(array('unionid'=>$post['unionid']));
				
			}
			
			$this->appReturn(array('status'=>true,'code'=>200,'msg'=>'用户存在'));
		}else{
			$this->appReturn(array('status'=>false,'code'=>201,'msg'=>'用户不存在','sql'=>$sql));
		}
    ...
    public function three(){
        ...
        if(!empty($result)){
			...
			if($res){
				...
				$this->appReturn(array('status'=>FALSE,'code'=>202,'msg'=>'当前手机号已存在'));
				
			}
			
			if(empty($post['yzm']) || $post['yzm'] != $yzm){
				
				$this->appReturn(array('status'=>FALSE,'code'=>202,'msg'=>'短信验证码有误','yzm1'=>$post['yzm'],'yzm2'=>$yzm));
    ...
    public function lawyer_info(){
		...
		$result[year] = $result['work_year'];
         if($result){
            $this->appReturn(array('status'=>true,'msg'=>'成功','code'=>200,'data'=>$result,'state'=>1));
        }else{
    ...
    public function news(){
        ...
        $start=$post[page]>0?($post[page]-1)*6: 0;
        ...
        $result=M('news')->where($where)->order('id desc')->limit($start,6)->select();
        ...
        if($result){
			...
			if($post['newsflag'] == 1){
				...
				//$this->appReturn(array('status'=>true,'code'=>200,'msg'=>'成功','data'=>''));
				$this->appReturn(array('status'=>true,'code'=>200,'msg'=>'成功','data'=>$result));
			}else{
    ...
    public function dt(){
        ...
        foreach($result as $k=>$v){
			...
			if(!empty($v['pid'])){
				...
				$dtInfo = M('dongtai')->where('id='.$v['pid'])->find();
				if(!empty($dtInfo)){
					$dtInfo[icons]= expic($dtInfo[icon]);
					if($dtInfo[utype]==1){
						$user=uinfo($dtInfo[uid]);
						$dtInfo[nickname]= yc_phone($user[phone]);
						$dtInfo[icon]= $user[icon];
					}else{
						$staff=sinfo($dtInfo[uid]);
						$dtInfo[nickname]= $staff[nickname].'律师';
						$dtInfo[icon]= $staff[icon];
					}

					$dtInfo[time]= date('Y-m-d H:i',$dtInfo[create_time]);
					
					
					$dtInfo[is_dz]==0;
					$dtInfo[zf]= M('dongtai')->where("pid=".$dtInfo[id])->count();
					$dtInfo[pl]= M('pl')->where("tid=".$dtInfo[id]." and type=2")->count();
					$dtInfo[dz]= M('dz')->where("tid=".$dtInfo[id]." and type=2")->count();
			...
			$result[$k]['zfInfo'] = empty($dtInfo)?'':$dtInfo;
        }
        $result[data]=$result;
    ...
    public function fask(){
        $post = $this->param;
        $start=$post[page]>0?($post[page]-1)*50 : 0;
		...
		if(!empty($post['fask_type'])){
			if($post['fask_type'] == 1){
				$where['_string'] = '((private=0 and reward_price=0) or ((private=1 and reward_price=0) and is_pay=1))';
			}else{
				$where['_string'] = '(private=1 or reward_price>0) and is_pay=1';
			}
			
		}else{
			
			$where['_string'] = '((private=0 and reward_price=0) or ((private=1 or reward_price>0) and is_pay=1))';
			
		}
		
        $where['status']=array('eq',1);
        $result=M('fask')->where($where)->order('sort desc,id desc')->limit($start,50)->select();
		$sql = M('fask')->getLastSql();
		$newList = array();
        ...
        foreach($result as $k=>$v){
			...
			$result[$k][time]=date('Y-m-d H:i',$v[create_time]);
            $result[$k][num]=M('fanswer')->where("tid=".$v[id].' and pid=0')->count();
        ...
        if($result){
            $this->appReturn(array('status'=>true,'code'=>200,'msg'=>'成功','data'=>$newList,'sql'=>$sql));
        }else{
            $this->appReturn(array('status'=>false ,'code'=>201,'msg'=>'失败','data'=>$result));
        }
    }
    public function askinfo(){
		...
		$result[time] = date('Y-m-d H:i',$result['create_time']);//format_date($result[create_time]);            
        if($result){
            $this->appReturn(array('status'=>true,'code'=>200,'msg'=>'成功','data'=>$result));
        }else{
    ...
    public function askinfo1(){
        ...
        $result[name]= empty($staff['nickname'])?$staff[phone]:$staff['nickname'];
        $result[icon]= $staff[icon];
        $result[ordertype]= ordertype($result[type]);
    ...
    public function faskdetail(){
        ...
        $result=M('fanswer')->where("tid=".$post['id'].' and pid=0')->order('id desc')->select();
        foreach($result as $k=>$v){
            $staff=sinfo($v['sid']);
            $result[$k][nickname]=$staff[nickname];
            $result[$k][icon]=$staff[icon];
		...
		$result['info']['time'] = date('Y-m-d H:i',$result['info'][create_time]);
    ...
    public function wt(){
        ...
        if($post['type']==1){
           /* $ww['w.status']=array('eq',1);
            //$ww['payed']=array('eq',0);
			$result = M('wt as w')->field('w.*')->where($ww)->order($order)->limit($start,4)->select();*/
			
			$ww['w.status']=array('eq',1);
			/*$map = array();
			$map['w.status'] = array('eq',2);
			$map['w.payed'] = array('eq',1);
			$map['w.uid']=array('eq',$post['uid']);
			$ww['_complex'] = $map;*/
			
			$ww1['_string'] = '(w.status=1 and w.uid='.$post['uid'].') or (w.status=2 and w.payed=1 and w.twopay=0 and w.uid='.$post['uid'].')';
            //$ww['payed']=array('eq',0);
			$result = M('wt as w')->field('w.*')->where($ww1)->order($order)->limit($start,20)->select();
			$sql = M('wt as w')->getLastSql();
			$newArray = array();
			foreach($result as $k => $v){
            ...
            $ww['w.status']=array('in','2,3,4');
            $ww['w.payed']=array('eq',1);
			$ww['w.twopay']=array('eq',0);
			$result = M('wt as w')->field('w.*')->where($ww)->order($order)->limit($start,10)->select();
        }else if($post['type']==3){
            $ww['w.status']=array('in','2,3,4');
            $ww['w.payed']=array('eq',1);
			$ww['w.twopay']=array('eq',1);
			$ww['w.is_confirm']=array('eq',0);
			$result = M('wt as w')->field('w.*')->where($ww)->order($order)->limit($start,10)->select();
        }else if($post['type']==4){
			...
			$ww['w.status']=array(array('egt',1),array('neq',3));
			$result = M('wt as w')->field('w.*')->where($ww)->order($order)->limit($start,10)->select();
        ...
        foreach($result as $k=>$v){
            ...
            $result[$k][phone]=empty($user['nickname'])?yc_phone($user[phone]):$user['nickname'];
            $region = M('region')->find($v['addrid']);
			if(!empty($region['pid'])){
				$region_parent = M('region')->find($region['pid']);
				
				if(!empty($region_parent['pid'])){
				$region_parent = M('region')->find($region_parent['pid']);
				
				
				}
			}
			
			$result[$k][addr]=$region_parent['name'].'-'.$region['name'];
        ...
        if($result){
            $this->appReturn(array('status'=>true,'code'=>200,'msg'=>$a,'data'=>$result,'sql'=>$sql));
        }else{
            $this->appReturn(array('status'=>false,'code'=>201,'msg'=>M('wt')->getLastSql(),'data'=>$ww));
        }
    ...
    public function wtdetail(){
        ...
        if(empty($result)){
			...
			$staffUid = array_column($result,'uid');
		
			$result['list']=$result;
		
			$result['is_tb'] = in_array($this->uid,$staffUid)?1:0;
			
		}

        $wt=M('wt')->find($post['id']);
		
		$xqlx = M('category')->where('id='.$wt['xqid'])->find();
		
		$wt['xqlx_title'] = empty($xqlx)?'普通代理':$xqlx['title'];
		
		$ajlx = M('category')->where('id='.$wt['ajid'])->find();
		
		$wt['ajlx_title'] = empty($ajlx)?'一般民事':$ajlx['name'];
    ...
    public function zc(){
        ...
        $result = M('zc')->where($where)->order('id desc')->limit($start,6)->select();
		...
		$total_price = D('Zongcou')->zcBalance($post['uid']);
		
        if($result){
            $this->appReturn(array('status'=>true,'code'=>200,'msg'=>'成功','data'=>$result,'total_price'=>$total_price));
        }else{
            $this->appReturn(array('status'=>false,'code'=>201,'msg'=>'失败','data'=>$result));
        }
    }
    ...
    public function zcdetail(){
		...
		$bili = D('Zongcou')->count_price($post[id],$result['money']); //众筹统计
    ...
    public function gonggao(){
		...
		if($post['type'] == 1){
			...
			$list = M('ad')->where('typeid=7 and status=1')->order('id desc')->limit($start,7)->select();
			
			if($list){
				$this->appReturn(array('status'=>true,'code'=>200,'msg'=>'成功','data'=>$list));
			}else{
				$this->appReturn(array('status'=>false ,'code'=>201,'msg'=>'失败','data'=>$list));
			}
    ...
    public function upload_file(){
        ...
        $upload->exts      =     array('doc', 'docx', 'ppt','pptx','xls','xlxs','pdf','txt');// 设置附件上传类型
        $upload->rootPath  =     './Uploads/'; // 设置附件上传根目录
        ...
        if(!$info) {// 上传错误提示错误信息
            ...
            if(!empty($vo)){
                ...
                $arr = array('code'=>200,'path'=>$pather,'video_id'=>$vo['id'],'name'=>$vo['name']);
