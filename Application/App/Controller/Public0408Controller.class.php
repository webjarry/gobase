...
class PublicController extends HomeController {
    ...
    public function reg()
    {
        ...
        if ($post['type'] == 1) {
            if ($result = M('usermember')->where($arr)->find()) {
                ...
                if($post['idno']){
                    if($pid = M('usermember')->where("idno=".$post['idno'])->find()['id']){
                        $add['pid'] = $pid;
                    }
                }
            ...
            if (M('staff')->where($arr)->find()) {
                ...
                if($post['idno']){
                    if($pid = M('staff')->where("idno=".$post['idno'])->find()['id']){
                        $add['pid'] = $pid;
                    }
                }
                $add['phone'] = $post['phone'];
    ...
    public function fask(){
        ...
        $where['status']=array('eq',1);
        $result=M('fask')->where($where)->order('id desc')->limit($start,10)->select();
        foreach($result as $k=>$v){
            $user=uinfo($v[uid]);
    ...
    public function wt(){
        ...
        if($post['addrid']>0){
            $ww['addrid']=array('eq',$post['addrid']);
        }
        if($post['ajid']>0){
            $ww['ajid']=array('eq',$post['ajid']);
        }
        if($post['uid']>0){
            $ww['uid']=array('eq',$post['uid']);
        }else{
            $ww['payed']=array('eq',1);
        }
        if($post['type']==1){
            ...
            $ww['status']=array('eq',2);
            $ww['payed']=array('eq',1);
        }else if($post['type']==3){
            $ww['status']=array('eq',3);
            $ww['payed']=array('eq',1);
        }else if($post['type']==4){
            $ww['status']=array('eq',4);
            $ww['payed']=array('eq',1);
        }

        $result = M('wt')->where($ww)->order($order)->limit($start,4)->select();$a=M('wt')->getLastSql();
        foreach($result as $k=>$v){
    ...
    public function paihang(){
        ...
        $start=$post[page]>0?($post[page]-1)*4 : 0;
        if($post['type']==1){
            $result=M('ask a')->join("bhy_staff s on a.sid=s.id")->field("s.*,count(a.sid) as snum,a.sid")->group('a.sid')->order('snum desc')->limit($start,4)->select();
        }else if($post['type']==2){
            $result=M('fanswer a')->join('bhy_staff s on a.sid=s.id')->field("s.*,count(*) as snum,a.sid")->group('a.sid')->order('snum desc')->limit($start,4)->select();
        }else if($post['type']==3){
            $result=M('mind a')->join('bhy_staff s on a.sid=s.id')->field("s.*,count(*) as snum,a.sid")->group('a.sid')->order('snum desc')->limit($start,4)->select();
        }
        if($result){
            foreach($result as $k=>$v){
                $staff=sinfo($v['id']);
