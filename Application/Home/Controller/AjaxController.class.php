           ...
           if(0){
               ...
               $map = array();$map['orderno'] = array('eq',$orderno);
				...
				if(!$order){
				  ...
				  // exit(json_encode(array('code'=>400,'msg'=>'测试！','order'=>$order)));
				   $re=M('order')->add($order);
					
				}else{
					
					M('order')->where('id='.$order['id'])->save(array('pay_type'=>$post['pay_type'],'update_time'=>time()));
					$re = $order['id'];
               ...
               if($re){
                   ...
                   if($post['type']==15){
					   ...
					   if($post['code_type'] != 1 || $post['type']==7){
						   $add['order_id'] = $re;
						   $add['orderno'] = $orderno;
						   $add['uid'] = $user['id'];
						   $add['sid'] = $post['sid'];
					   ...
					   if($post['type']==3 && $post['is_urgent']==2){ //紧急订单
					   
							$map = array();$map['orderno'] = array('eq',$orderno);
						    M('ask')->where($map)->save(array('money'=>$post[money],'is_urgent'=>1));
    ...
    public function checkorder(){
        if(IS_POST){
            ...
            if($order){
				...
				if($order['status'] == 1 && $order['type'] == 14){
					
					exit(json_encode(array('code'=>202)));
				}
