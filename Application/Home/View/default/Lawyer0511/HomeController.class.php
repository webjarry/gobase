...
class HomeController extends Controller {
    ...
    public function lists(){
        ...
        if($post[page]>0){
            $result=$m->where($where)->order('id desc')->limit($start,$pagesize)->select();
        }else{
        ...
        if($result){
            $this->appReturn(array('status'=>true,'code'=>200,'msg'=>$m->getLastSql(),'data'=>$result,'count'=>count($result)));
        }else{
            $this->appReturn(array('status'=>false ,'code'=>201,'msg'=>'失败','data'=>$result));
        }
    }
    ...
    public function checkIndividual() {
        if ($this->uid == 0) {
            ...
            if($this->infock==1){
                ...
                if($pass==0){
                    $this->appReturn(array('status' => false, 'msg' => '您的个人信息尚未认证，无法进行该操作', 'code' => 501));
                } 
            }
